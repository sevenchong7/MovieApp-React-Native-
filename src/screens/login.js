...
const Login = ({navigation}) => {
   ...
   useEffect(() => { setOTPView( randomStr() ) },[]) //1st time load

   function reGetOPT(){//funtion 
    return setOTPView(randomStr);
   };
    ...
    function check_user(){//function
        ...
        for (i = 0 ; i <= user.length; i++){//loop to get array data
            if (user.find(user=>user.username===userName) && user.find(user=>user.password===passWord)){//compare the data and input
                ...
                <SafeAreaView style= {styles.content_container}>
                    ...
                    <View style={styles.OTP_input}>
                        <View style={styles.OTPInput}>
                            <Image style={styles.user_logo} source={require("../assets/security_pin.png")} />
